...
const SOURCE = path.join(process.cwd(), "photos", "Baskets", "salomon", "XT-QUEST ADV");
...
const VARIANTS: ColorVariant[] = [
  { color: "Black White GTX",     hex: "#1a1a1a", fileIds: range(608, 616) },
  { color: "Navy Silver Vintage", hex: "#1e2a3a", fileIds: range(617, 625) },
  { color: "Triple Black XA Pro", hex: "#000000", fileIds: range(626, 634) },
  { color: "White Red Lab",       hex: "#e8e8e8", fileIds: range(635, 643) },
  { color: "Mint Silver",         hex: "#cfe6d8", fileIds: range(644, 652) },
  { color: "Navy Silver Lab",     hex: "#243042", fileIds: range(653, 661) },
  { color: "Black Red GTX",       hex: "#2a1a1a", fileIds: range(662, 670) },
];

const PRIMARY_COLOR = "Black White GTX";
const PRIMARY_FILE = "608";
...
async function main() {
  ...
  const product = await prisma.product.create({
    data: {
      ...
      sku: SKU,
      description:
        "XT-Quest ADV — silhouette trail tactique, accroche Contagrip, châssis SensiFit + Endofit. Look outdoor premium pour la ville comme pour le sentier.",
      brandId: brand.id,
      gender: "UNISEX",
      terrain: "TRAIL",
      cushionLevel: "MEDIUM",
  ...
  for (const v of VARIANTS) {
    for (const id of v.fileIds) {
      ...
      const url = await uploadImage(abs, `xtquest-${v.color.replace(/\s+/g, "-").toLowerCase()}-${id}.jpg`);
      imageRecords.push({
