...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${category.name} — Chaussures de Running`,
    description: category.description ?? `Shop all ${category.name} running shoes.`,
  };
}

export default async function ShoeCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const category = await getCategory(slug);
    ...
    <>
      ...
      {products.length === 0 ? (
        <div className="max-w-[1440px] mx-auto px-4 lg:px-8 py-20 text-center text-gray-400">
          ...
          <p className="text-sm mt-1">Ajoute des produits via le panel admin et assigne cette catégorie</p>
        </div>
      ) : (
